(function loader() {
  ...
  const win = window,
        doc = win.document,
        wrap = find('[data-r~=wrap]'),
        field = find('[data-r~=field]'),
        results = find('[data-r~=results]'),
        leftBtn = find('[data-r~=leftBtn]'),
        rightBtn = find('[data-r~=rightBtn]'),
        slider = find('[data-r~=slider]'),
        oa = [
          0,
          0,
          ['D1, "the madhouse"'],
  ...
  function initStorage() {
    if ((ls = win.localStorage)) {
      try {
        lsAlpha = ls.getItem('cc.alpha');
        lsNotes = ls.getItem('cc.notes');
        lsScore = ls.getItem('cc.score');
      } catch(err) {
        ls = 0;
  ...
  function initToggleButtons() {
    lsAlpha !== '0' && addState(wrap, 'alpha');
    lsNotes !== '0' && addState(wrap, 'notes');
    find('[data-r~=alphaBtn]').addEventListener('click', () => {
      clickAlphaButton();
      generateOutput();
    });
    find('[data-r~=notesBtn]').addEventListener('click', () => {
      clickNotesButton();
    });
  }
  function initIncrementalButtons() {
    leftBtn.addEventListener('click', () => {
      leftBtn.matches('[data-s~=enabled]') && clickIncrementalButton('l');
    });
    rightBtn.addEventListener('click', () => {
      rightBtn.matches('[data-s~=enabled]') && clickIncrementalButton('r');
    });
  }
  function initSlider() {
  ...
  function initField() {
    lsScore ? (field.value = lsScore) && generateOutput() : populateRandomNumber();
    field.addEventListener('input', (e) => {
      generateOutput(e);
    });
  }
  function initRandomButton() {
    find('[data-r~=randomBtn]').addEventListener('click', () => {
      wrap.matches('[data-s~=mapMode]') ? removeState(wrap, 'mapMode') : populateRandomNumber();
    });
  }
  function initMapLink() {
    find('[data-r~=mapLink]').addEventListener('click', () => {
      clickMapLink();
    });
  }
  function initMap() {
  ...
  //
  function clickAlphaButton() {
    toggleState(wrap, 'alpha');
    ls && ls.setItem('cc.alpha', wrap.matches('[data-s~=alpha]') ? 1 : 0);
  }
  function clickNotesButton() {
    toggleState(wrap, 'notes');
    ls && ls.setItem('cc.notes', wrap.matches('[data-s~=notes]') ? 1 : 0);
  }
  function clickIncrementalButton(d) {
    let v = parseInt(field.value);
    field.value = d === 'r' ? ++v : --v;
    generateOutput();
  }
  function manageIncrementalBtnStates(v) {
    manageAttrVal(leftBtn, 'data-s', 'enabled', !!(v && v > 2));
    manageAttrVal(rightBtn, 'data-s', 'enabled', !!(v && v < 170));
  }
  function clickMapLink() {
    !wrap.matches('[data-s~=mapRendered]') && initMap();
    toggleState(wrap, 'mapMode');
  }
    ...
    if (o) {
      ...
      o.forEach((lines) => {
        ...
        for (let i = 0; i < lines.length; i++) {
          ...
          html += '<span class="' + clss + '">' + line + '</span>';
        }
        html += '</span>';
      });
      if (footnote) {
      ...
      addState(wrap, 'results');
      field.value = slider.value = v;
      ls && ls.setItem('cc.score', v);
      results.innerHTML = html;
    }
    else {
      removeState(wrap, 'results');
      results.innerHTML = '';
    }
    manageIncrementalBtnStates(o && v);
  }
  function convertAlphanumeric(l) {
    let d, t;
    l.split(/[\s-]+/).forEach(() => {
      if ((d = l.match(/D[^a-z][0-9]*/))) {
        l = l.replace(/D[0-9]*/, parseInt(d[0].substr(1) * 2));
      }
      if ((t = l.match(/T[^a-z][0-9]*/))) {
        l = l.replace(/T[0-9]*/, parseInt(t[0].substr(1) * 3));
      }
    });
    return l;
  }
  ...
  //
  function find(sl) {
    return doc.querySelector(sl);
  }
  function addState(el, v) {
    manageAttrVal(el, 'data-s', v, 1);
  }
  function removeState(el, v) {
    manageAttrVal(el, 'data-s', v, 0);
  }
  function toggleState(el, v) {
    manageAttrVal(el, 'data-s', v);
  }
  function manageAttrVal(el, a, v, f) {
